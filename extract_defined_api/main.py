...
def _locate(path, forceload=False):
...
def search_functions_and_classes(the_object) -> Tuple[list, list, list]:
    ...
    data = []
    for key, value in inspect.getmembers(the_object, pydoc.isdata):
        if _visiblename(key, the_all, the_object):
            data.append((key, the_object.__name__))
...
def locate(the_object_name: str):
    ...
    """
    try:
        return sys.modules[the_object_name]
    except KeyError:
        pass

    try:
        return pydoc.locate(the_object_name, forceload=False)  # 从 pydoc.writedoc 扒出来的
    except (ImportError, pydoc.ErrorDuringImport) as e:  # torch._storage_docs torch.for_onnx
        # https://stackoverflow.com/questions/60257756/unable-to-import-torch-distributed-rpc
        if "\'torch._C\' is not a package" in str(e) or "from 'torch.distributed.rpc'" in str(e):
            pass
        else:
            print(f"!!!!!!2 {e} {the_object_name=}")
